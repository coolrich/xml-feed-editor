...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, app):
        ...
        self.source_products_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        ...
        self.final_products_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        ...
        self.add_cat_push_button.clicked.connect(
            lambda: self.move_categories_between_tables(self.source_category_table_view, self.final_category_table_view))
        # self.delete_cat_push_button.clicked.connect(self.remove_category_from_table)
        self.delete_cat_push_button.clicked.connect(
            lambda: self.move_categories_between_tables(self.final_category_table_view, self.source_category_table_view))
        self.fin_cat_model.rowsInserted.connect(self.add_products_to_src_products_table)
        self.add_prod_push_button.clicked.connect(self.move_products_from_source_destination_table)
        self.fin_cat_model.rowsAboutToBeRemoved.connect(self.refresh_products_in_products_table)
        self.xml_data = None
    ...
    def move_products_from_source_destination_table(self):
        source_model = self.source_products_table_view.model()
        destination_model = self.final_products_table_view.model()
    ...
    def refresh_products_in_products_table(self, parent, start, end):
        sptv_model = self.source_products_table_view.model()
        # fctv_model = self.final_category_table_view.model().sourceModel()
    ...
    def find_in_src_products(self):
        pass

    def find_in_fin_products(self):
        pass

    @staticmethod
    def move_categories_between_tables(source_table_view: QTableView,
